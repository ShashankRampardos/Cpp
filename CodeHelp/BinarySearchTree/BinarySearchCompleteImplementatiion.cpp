...
//deletion may bhi same logic hay search ka
...
//delete node , assuming all elements are unique in tree.
Bst* deleteNode(Bst* root,int target){
    if(root==NULL)
      return nullptr;
  //ek case is bar bada hay kafi 
  if(root->data==target){
    ...
    //ek case solved baki recursion ko bol, ( ye nichay searching ka logic hay)
