    vector<int> topView(Node *root)
    {
      ...
      //level order tarversal
        while(!q.empty()){
          ...
          //check if there is a entry for horizontalDistance in map, if not then insert current node data
            if(mp.find(horizontalDistance)==mp.end())
              mp[horizontalDistance]=rootNode->data;
          //in bottom view just remove above condition and let map be overwrittne, rest code will be same.
